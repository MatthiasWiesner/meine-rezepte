function MeineRezepte() {
    ...
    this.initTemplates = function () {
        ...
        self.recipeUpdateinitNewTagTemplate = $('#recipe-update-new-tag-template').html();
    ...
    this.initiateTagList = function () {
        ...
        self.backend.getTagList(function (tags) {
            ...
            $('.tag-list-item-link').on('click', self.displayAllRecipesByTagFilter);
            $('.tag-delete-item').on('click', self.onDeleteTag);
        });
    }
    ...
    this.initNewTag = function () {
        $('#tag-new').val('');
        $('#tag-new').unbind('keypress');
        $('#tag-new').keypress(function (e) {
            if (e.which == 13) {
                var value = $(this).val();
                if (value.length == 0) {
                    alert("Bitte ein Tag Name angeben");
                } else {
                    self.backend.addTag(value, self.run);
                }
            }
        });
    }
    ...
    this.onDeleteTag = function () {
        var tagId = $(this).attr('data-tag-id');
        self.backend.deleteTag(tagId, self.run);
    }
